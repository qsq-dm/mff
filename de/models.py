...
class ItemComment(Model):
    ...
    def as_dict(self):
        return dict(
            ...
            photos          = self.photos,
            photo_list      = prefix_img_list(self.photos),
            thumb_photo_list= prefix_img_list_thumb(self.photos),
...
class Qrcode(Model):
    ...
    image              = db.Column(String(300))
    act_type           = db.Column(Integer, default=0) #推广活动类型
...
class RoomDesignDetail(Model):
    ...
    def as_dict(self):
        return dict(
            ...
            pics        = self.pics,
            create_time = self.create_time,
            pic_list    = prefix_img_list(self.pics)
        )
