...
def get_hospital_cats():
    ...
    user                = HospitalService.get_user_by_name(request.name)
    hospital_id         = user.hospital_id
    sub_cat_ids         = HospitalService.get_hospital_sub_cat_ids(hospital_id)

    sub_cat_count_map   = dict(ItemService.count_sub_cat_items())

    sub_cat_list        = ItemService.get_subcats_by_ids(sub_cat_ids)
    cat_ids             = [i['cat_id'] for i in sub_cat_list]
    ...
    for cat in cat_list:
        sub_cats        = [i for i in sub_cat_list if i['cat_id']==cat['id']]
...
def cat():
    ...
    sub_cat_count_map   = dict(ItemService.count_sub_cat_items(hospital_id))
...
def cat_items():
    ...
    if cat_id:
        subquery    = db.session.query(ItemSubCat.id).filter(ItemSubCat.cat_id==cat_id).subquery()
        where       = Item.sub_cat_id.in_(subquery)
    elif sub_cat_id:
        where       = Item.sub_cat_id==sub_cat_id
    if keyword:
        where       = or_(
            Item.id==keyword,
            and_(
